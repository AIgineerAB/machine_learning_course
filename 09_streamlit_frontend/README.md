# Building a streamlit frontend to consume CRUD API


**Streamlit frontend**

<a href="https://youtu.be/nGRtRqZSVkY" target="_blank">
  <img src="https://github.com/kokchun/assets/blob/main/fastapi/streamlit_fastapi.png?raw=true" alt="pydantic for data validation" width="600">
</a>

In the previous lecture we built a CRUD API, now it's time to consume it using a Streamlit frontend. This makes the app fullstack.


## Read more
- [Streamlit documentation](https://docs.streamlit.io/)

## Other videos